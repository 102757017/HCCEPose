...
set GPU_ID=0
set SCENE_NUM=2
set CC0TEXTURES=..\cc0textures-512
...
REM 训练 YOLO 模型
python s3_p2_train_yolo.py --dataset_path %DATASET_PATH% --gpu_num 2 --batch_size 8 --epochs 1

REM 物体正背面标签制备
python s4_p1_gen_bf_labels.py --dataset_path %DATASET_PATH%

REM 训练 HccePose（分布式）
python -m torch.distributed.launch --nproc_per_node=2 s4_p2_train_bf_pbr_ddp.py --dataset_path %DATASET_PATH% --start_obj_id 1 --end_obj_id 1 --total_iteration 10
