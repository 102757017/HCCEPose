...
# 【修复：显式传入 CUDA_DEVICE】避免依赖不安全的全局作用域变量
...
if __name__ == '__main__':
    ...
    for obj_id in range(start_obj_id, end_obj_id + 1):
        ...
        # ---------------------------------------------------------------------
        # 【修复1】必须在传递参数给优化器之前进行 SyncBatchNorm 转换
        # 否则优化器将保留旧的 BatchNorm 参数而不是 SyncBatchNorm 的参数
        # ---------------------------------------------------------------------
        if not ide_debug:
        ...
        # ---------------------------------------------------------------------
        # 【修复2】在使用 DDP 时必须使用 DistributedSampler，否则每张卡加载的数据会完全相同
        # ---------------------------------------------------------------------
        if not ide_debug:
            train_sampler = torch.utils.data.distributed.DistributedSampler(train_bop_dataset_back_front_item)
        ...
        while True:
            ...
            # 【修复2配套逻辑】在使用 DistributedSampler 时，必须在每个 epoch 打乱数据
            if not ide_debug:
            ...
            for batch_idx, (rgb_c, mask_vis_c, GT_Front_hcce, GT_Back_hcce) in enumerate(train_loader):
                ...
                if args.local_rank == 0:
                    if (iteration_step)%log_freq == 0 and iteration_step > 0:
                        ...
                        # 【修复：调用 test 函数时显式传入 CUDA_DEVICE】
                ...
                if not ide_debug:
                    ...
                    if nan_flag.item() > 0:
                        # ---------------------------------------------------------------------
                        # 【修复3】避免因 net 被 DDP 包装后无 model 属性导致崩溃
                        # 并且改用 torch.nn.modules.batchnorm._BatchNorm 以兼容 SyncBatchNorm
                        # ---------------------------------------------------------------------
                        target_net = net.module if isinstance(net, torch.nn.parallel.DistributedDataParallel) else net
