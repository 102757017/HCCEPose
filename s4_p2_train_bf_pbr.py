    ...
    for batch_idx, (rgb_c, mask_vis_c, GT_Front_hcce, GT_Back_hcce, Bbox, cam_K, cam_R_m2c, cam_t_m2c) in tqdm(enumerate(test_loader)):
        ...
        with autocast():
            ...
            for (cam_R_m2c_i, cam_t_m2c_i, pred_m_bf_c_np_i) in zip(cam_R_m2c.detach().cpu().numpy(), cam_t_m2c.detach().cpu().numpy(), pred_m_bf_c_np):
                ...
                add_list_l.append(add_list)
        torch.cuda.empty_cache()
>>>>>>> 3e96c42eeb7f6dad9539ba822799069069aa2cef
...
<<<<<<< HEAD
    ...
    warmup_steps = 100
=======
    ide_debug = False          # 改为 False 启用 DDP

    dataset_path = './demo-bin-picking'
    train_folder_name = 'train_pbr'
    start_obj_id = 1
    end_obj_id = 1
    total_iteration = 12501
    lr = 0.0008
    batch_size = 48
    test_batch_size = 24
    num_workers = 4
    ...
    # 迁移学习配置（可选）
    transfer_weight_path = r'./pre-trained/0_8283step50000'
    warmup_steps = 70        # 线性预热步数
>>>>>>> 3e96c42eeb7f6dad9539ba822799069069aa2cef
...
=======
        ...
        min_xyz = torch.tensor([obj_info['min_x'], obj_info['min_y'], obj_info['min_z']], dtype=torch.float32).to('cuda:'+CUDA_DEVICE)
        size_xyz = torch.tensor([obj_info['size_x'], obj_info['size_y'], obj_info['size_z']], dtype=torch.float32).to('cuda:'+CUDA_DEVICE)

        loss_net = HccePose_Loss()
        scaler = GradScaler()
        net = HccePose_BF_Net(efficientnet_key=efficientnet_key, input_channels=3, min_xyz=min_xyz, size_xyz=size_xyz)
        net_test = HccePose_BF_Net(efficientnet_key=efficientnet_key, input_channels=3, min_xyz=min_xyz, size_xyz=size_xyz)
        if torch.cuda.is_available():
            net = net.to('cuda:'+CUDA_DEVICE)
            net_test = net_test.to('cuda:'+CUDA_DEVICE)
        ...
        # ===== 修复：直接调用 load_checkpoint 而不使用 try-except =====
        ...
        if iteration_step > 0:
            if local_rank == 0:
                print(f"==> Resumed from checkpoint at step {iteration_step}")
            for _ in range(iteration_step):
                scheduler.step()
        else:
            if local_rank == 0:
                print('No local checkpoint found.')
            if transfer_weight_path is not None and os.path.exists(transfer_weight_path):
                if local_rank == 0:
                    print(f"==> Loading transfer weights from {transfer_weight_path}")
                checkpoint = torch.load(transfer_weight_path, map_location='cpu', weights_only=False)
                state_dict = checkpoint['model_state_dict']
                state_dict = {k: v for k, v in state_dict.items() if 'aspp.conv_1x1_4' not in k}
                net.load_state_dict(state_dict, strict=False)
            else:
                if local_rank == 0:
                    print("No transfer weights provided or file not found, training from scratch.")

        # DDP 包装必须在所有 load_state_dict 之后
        if not ide_debug:
            net = torch.nn.SyncBatchNorm.convert_sync_batchnorm(net)
            net = torch.nn.parallel.DistributedDataParallel(net, device_ids=[args.local_rank])
...
<<<<<<< HEAD
                ...
                # 分布式 NaN 处理
=======
                    loss = 3 * torch.sum(current_loss['Front_L1Losses']) + \
                           3 * torch.sum(current_loss['Back_L1Losses']) + \
                           current_loss['mask_loss']
...
=======
                ...
                if args.local_rank == 0:
                    print(f"dataset:{os.path.basename(dataset_path)} - obj{str(obj_id).rjust(2,'0')}",
...
=======
                    print('end training at iteration_step:', iteration_step)
>>>>>>> 3e96c42eeb7f6dad9539ba822799069069aa2cef
                break
